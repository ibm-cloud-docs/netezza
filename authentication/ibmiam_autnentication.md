...
1. [Connect to {{site.data.keyword.netezza_short}}](/docs/netezza?topic=netezza-connecting-overview) as an `admin` user.
   ...
   | password       | **When MFA is not configured:** \n specify the `access-key` and `secret-key` for the user. \n **When MFA is configured:**, specify the `access-key`, `secret-key`, and `mfa-code`. |


   When MFA is not configured specify the access-key and secret-key.
When MFA is configured specify the access-key, secret-key, and mfa-code for the user.
